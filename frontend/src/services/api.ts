...
api.interceptors.request.use(
  async (config) => {
    ...
    if (config.data instanceof FormData) {
      console.log(
        "🔧 DEBUG: FormData detected, removing Content-Type header for multipart upload",
      );
...
function scheduleSessionTimers() {
  ...
  const remainingUntilTimeout = Math.max(
    INACTIVITY_TIMEOUT_MS - timeSinceActivity,
    0,
  );
  const warningDelay = Math.max(
    remainingUntilTimeout - WARNING_BEFORE_EXPIRY_MS,
    0,
  );

  // Schedule warning before expiry
  warningTimer = globalThis.setTimeout(async () => {
    const remainingMs = Math.max(
      INACTIVITY_TIMEOUT_MS - (Date.now() - lastUserActivityAt),
      0,
    );
    ...
    const stayLoggedIn = globalThis.confirm(
...
if (eventTarget) {
  ...
  eventTarget.addEventListener("keydown", handleUserActivity);
  eventTarget.addEventListener("mousemove", handleUserActivity);
  eventTarget.addEventListener("scroll", handleUserActivity);
